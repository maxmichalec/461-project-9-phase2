...
export class PackageComponent {
	...
	updateMessage = ''
	deleteMessage = ''
	postPackageData: PackageData = {
		//User Input for putPackage
		Content: '',
		URL: '',
		JSProgram: '',
	}
	postPackageDataResponse: Package = {
		//Response from putPackage
		data: {
			Content: '',
			URL: '',
			JSProgram: '',
	...
	getPackage() {
		this.apiService
			.packageRetrieve({ id: this.packageId, 'X-Authorization': this.authHeader })
			.subscribe(
				(response) => {
	...
	updatePackage() {
		...
		this.apiService
			.packageUpdate({
				id: this.packageData.metadata.ID,
	...
	deletePackage() {
		this.deleteMessage = ''
		this.apiService
			.packageDelete({ id: this.deletePackageId, 'X-Authorization': this.authHeader })
			.subscribe(
				(response) => {
	...
	putPackage() {
		this.apiService
			.packageCreate({ 'X-Authorization': this.authHeader, body: this.postPackageData })
			.subscribe(
				(response) => {
	...
	ratePackage() {
		this.apiService
			.packageRate({ 'X-Authorization': this.authHeader, id: this.ratePackageId })
			.subscribe(
				(response) => {
