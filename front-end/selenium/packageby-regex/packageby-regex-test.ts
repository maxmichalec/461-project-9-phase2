...
export async function packageByRegexTest(driver: WebDriver): Promise<void> {
	try {
		...
		const searchPackagesButton = await driver.wait(
			until.elementLocated(By.css('.package-search-container button')),
			5000,
		);
		await searchPackagesButton.click();

		// Wait for the output or changes caused by clicking the button
		const searchResponse = await driver.wait(
			until.elementLocated(By.css('.package-list')),
			5000,
		);
