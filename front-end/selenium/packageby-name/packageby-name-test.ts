...
export async function packageByNameTest(driver: WebDriver): Promise<void> {
	try {
		...
		const packageNameInput = await driver.findElement(
			By.css('.package-by-name-container input'),
		);
		...
		const historyResponse = await driver.wait(
			until.elementLocated(By.css('.history-entry')),
			5000,
		);
