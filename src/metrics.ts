...
export class PullRequests extends BaseMetric {
	...
	description =
		'Measures the fraction of code introduced through pull requests with code reviews.'
	...
	async getPullRequestsWithCodeReviews(): Promise<number> {
		//this code currently considers Pull Request with review comments as having a code review
		try {
			const pullRequests = await this.octokit.rest.pulls.list({
				owner: this.owner,
				repo: this.repo,
...
export class DependencyPins extends BaseMetric {
	...
	async evaluate(): Promise<number> {
		try {
			...
			if (Array.isArray(packageJson.data)) {
				...
				const numDependencies = packageJsonParsed.dependencies
					? Object.keys(packageJsonParsed.dependencies)?.length
					: undefined
				if (numDependencies === undefined || numDependencies === 0) {
					// return 0 on undefined?
					return 1
				}
	...
	private numPinnedDeps(dependencies: string[]): number {
		...
		for (const dependency in dependencies) {
			...
			if (
				/^(?:\d+\.\d+\.\d+|\d+\.\d+(\.[\d+*Xx])?|~\d+\.\d+(\.\d+)?|\^0\.\d+(\.\d+)?|\d+\.\d+(\.[*Xx])?)$/.test(
					version,
				)
			) {
				pinnedDeps++
