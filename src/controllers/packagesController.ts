...
export const getPackages = async (req: Request, res: Response) => {
	try {
		...
		const authToken: AuthenticationToken | undefined = Array.isArray(
		...
		const results = await Promise.all(
			packageQueries.map(async ({ Name, Version }) => {
				...
				if (Name === '*') {
					...
					const allPackagesResult = await dynamoDb.send(
						new ScanCommand({
							...
							ExclusiveStartKey: offset
								? unmarshall(JSON.parse(Buffer.from(offset, 'base64').toString()))
								: undefined,
						}),
					);
						...
						if (exactMatch) {
							...
							const params = {
								...
								FilterExpression:
									'#n = :name' +
									(Version
										? ' AND #v BETWEEN :startVersion AND :endVersion'
										: ''),
								ExpressionAttributeNames: {
									'#n': 'name', // Attribute name for 'Name'
									...(Version ? { '#v': 'version' } : {}), // Add 'Version' attribute name if a version is provided
								...
								ExpressionAttributeValues: marshall({
									...
									...(Version
										? {
												':startVersion': boundedRange[1],
												':endVersion': boundedRange[2],
										  }
										: {}),
								}),
								ExclusiveStartKey: offset
							...
							const params = {
								...
								FilterExpression:
									'#n = :name' +
									(Version
										? ' AND #v >= :startVersion AND #v < :endVersion'
										: ''),
								ExpressionAttributeNames: {
									'#n': 'name', // Attribute name for 'Name'
									...(Version ? { '#v': 'version' } : {}), // Add 'Version' attribute name if a version is provided
